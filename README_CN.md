...
- **深度内容理解**: 自动解析并切分上传的文档 (PDF, DOCX, TXT, MD)，提取时间数据、关键实体和关系。
- **持久化记忆**: 分析结果持久化存储在数据库中，允许 AI 在页面刷新和切换会话后保留上下文。
- **时间上下文感知**: 所有智能体都能感知当前的日期和时间，从而实现精确的时间线生成和日程规划。
...
- **响应式数据表格**: 所有数据表格（在聊天和分析卡片中）都包裹在具有高级样式的响应式容器中，确保布局永不崩坏。
- **可视化加载状态**: 在加载历史记录和生成内容期间提供清晰的视觉反馈。
...
- [x] 消息分支与多版本导航逻辑
- [x] 一键新建会话 (上下文重置)
- [x] 增强型多模态：支持非图片文件 (PDF, Docx 等) 解析
- [x] 智能文档分析与切分
